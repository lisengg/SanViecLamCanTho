...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
			...
			try {
				...
				if (tkDoanhNghiep == null) {
					if (tkNLD.getIDTAIKHOAN() != null) {
						...
						hoten = tk.getIDTAIKHOAN().toString();
						password = pe.encode(tk.getMATKHAU());
						roles = "NLD";
						if (tk != null) {
							session.setAttribute("user", tk.getIDTAIKHOAN());
					...
					if (tkDoanhNghiep.getIDTAIKHOANDN() != null) {
						...
						hoten = tk.getIDTAIKHOANDN().toString();
						password = pe.encode(tk.getMATKHAU());
						roles = "DOANHNGHIEP";
						if (tk != null) {
							session.setAttribute("user", tk.getIDTAIKHOANDN());
							session.setAttribute("roles", roles);
						}
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable();
		// Phân quyền sử dụng
		http.authorizeRequests().antMatchers("/qlisan")
