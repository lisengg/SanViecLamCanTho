...
public class DoanhNghiepController {
	...
	public String chinhsuatk(Model model) {
		...
		if(sessionRole.equals("DOANHNGHIEP")){
			TaiKhoanDoanhNghiep userDN = doanhNghiepService.findById(Integer.parseInt(sessionUser));
			model.addAttribute("user",userDN);
		}
		
		return "chinhsuatkdoanhnghiep";
	}
	
	@RequestMapping("/baomatdn")
	public String baomattk(Model model) {
		String sessionUser = session.getAttribute("user").toString();
		String sessionRole = session.getAttribute("roles").toString();
		if(sessionRole.equals("DOANHNGHIEP")){
			TaiKhoanDoanhNghiep userDN = doanhNghiepService.findById(Integer.parseInt(sessionUser));
